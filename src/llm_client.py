...
Uses Groq API with retry logic and exponential backoff
...
# Model — updated from deprecated llama-3.1-8b-instant
...
def analyze_chunk(client: Groq, chunk: dict) -> dict:
    ...
    try:
        ...
        logger.error(f"All retries exhausted for chunk {idx}: {e}")
        ...
        logger.error(f"JSON parsing failed for chunk {idx}: {e}")
        ...
        logger.error(f"Unexpected error for chunk {idx}: {e}")
...
def analyze_all(chunks: list) -> list:
    ...
    logger.info(f"LLM analysis complete: {success}/{len(results)} success")
    return results
